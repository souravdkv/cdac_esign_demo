...
//     request.getSession().setAttribute("fileName","/Signed_Pdf.pdf");
			...
			String user = null;
			rectangle = new Rectangle(cropBox.getRight(), cropBox.getBottom(), cropBox.getRight(100),
...
//				appearance.setVisibleSignature(rectangle, i, null);
//			}
			appearance.setVisibleSignature(rectangle, reader.getNumberOfPages(), null);

//			for (int i = 1; i <= reader.getNumberOfPages(); i++) {
//                appearance.setVisibleSignature(new Rectangle(10, 10, 50, 50), i, "signature" + i);
//                stamper.close();
//                reader = new PdfReader(sourcefile);
...
//            }
	...
	public String signPdfwithDS(String response, HttpServletRequest request, HttpSession session) {
		...
		// PdfSignatureAppearance appearance =
		// (PdfSignatureAppearance)request.getSession().getAttribute("appearance");
		int contentEstimated = 8192;
		try {
			...
			// PdfSignatureAppearance appearance =
			// (PdfSignatureAppearance)request.getSession().getAttribute("pdfHash");
